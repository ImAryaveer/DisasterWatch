  ...
  useEffect(() => {
    const fetchDisasters = async () => {
      try {
        ...
        let disasterData = response.data;
        
        // Handle the new response format
        if (response.data.isMockData) {
          disasterData = response.data.disasters;
          setUsingMockData(true);
        } else {
          setUsingMockData(false);
        }
        
        if (disasterData && Array.isArray(disasterData) && disasterData.length > 0) {
          // Process and validate the data
          const validData = disasterData.filter(item => 
            item.location && 
          ...
          console.log("Using data:", validData.length, "valid disasters");
        } else {
