...
router.get('/', async (req, res) => {
  try {
    ...
    console.log("Received response from GDACS");
    ...
    if (!result.rss || !result.rss.channel || !result.rss.channel.item) {
      console.log("Invalid XML structure, using mock data");
      return res.json({
        disasters: getMockDisasterData(),
        isMockData: true
      });
    }
    ...
    if (realDisasters.length === 0) {
      console.log("No valid disasters found in GDACS data, using mock data");
      return res.json({
        disasters: getMockDisasterData(),
        isMockData: true
      });
    ...
    console.log(`Successfully processed ${realDisasters.length} real disasters`);
    res.json(realDisasters);
  } catch (error) {
